...
final case class Position(x: Int, y: Int, z: Int, w: Int) {
  def +(p2: Position) = Position(x + p2.x, y + p2.y, z + p2.z, w + p2.w)
}
...
object Grid {
  def range(min: Int, max: Int) = {
    val rs = min to max
    for (x <- rs; y <- rs; z <- rs; w <- rs) yield Position(x, y, z, w)
  }
